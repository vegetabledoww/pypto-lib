...
# A source contributes at most one row per token to any expert. Packing two
# base-129 counts leaves enough INT32 headroom for all B4 x 8192 prefill epochs.
META_PACK_BASE = MAX_PER_SRC + 1
META_COUNTS_PER_WORD = 2
...
def dispatch(
    ...
    with pl.at(
        ...
        for dst in pl.range(N_RANKS):
            if packed_meta != 0:
                # Wait for the receiver to consume this source's previous epoch
                # before overwriting the exact metadata words it will read.
                if dst != my_rank:
                    pld.system.wait(
                        signal=arrived,
                        offsets=[dst, 0],
                    ...
                    for word in pl.range(META_PACK_WORDS):
                        ...
                        encoded_count = pl.cast(
                            moe_epoch * META_EPOCH_STRIDE + packed, pl.INT32
                        )
                        pld.system.notify(
                            target=recv_meta,
                            peer=dst,
        ...
        if packed_meta != 0:
            # Wait on the exact metadata words being consumed. A separate notify
            # does not order a remote_store to a different address.
                ...
                for lane in pl.range(META_COUNTS_PER_WORD):
                    ...
                    for src in pl.range(N_RANKS):
                        if src == my_rank:
                            ...
                            count = pl.cast(
                                (packed // divisor) % META_PACK_BASE, pl.INT32
                            )
                        pl.write(recv_meta_local, [src, e], count)
                        acc = acc + count
                    pl.write(recv_count_out, [e, 0], acc)

            # Acknowledge only after all words from each source have been decoded.
            for src in pl.range(N_RANKS):
                if src != my_rank:
                    pld.system.notify(
                        target=arrived,
    ...
    # Do not reuse a peer's payload lane until it has consumed the prior epoch.
    with pl.at(
        level=pl.Level.CORE_GROUP,
        name_hint="dispatch_reuse_wait",
        allow_early_resolve=True,
    ) as _reuse_tid:
        # This data anchor prevents future-epoch waits from reserving AIV cores
        # before the current gate chain can produce the corresponding ACKs.
            ...
            for k in pl.range(TOPK):
                ...
                if le == loc_e:
                    ...
                    if draining_payload != 0:
                        ...
                        pl.tile.write(
                            aux_tile, [0, AUX_SCALE], pl.read(x_norm_scale, [t, 0])
                        )
                        pl.tile.write(aux_tile, [0, AUX_W], pl.read(weights, [t, k]))
                        pld.tile.remote_store(
                            aux_tile, target=recv_aux, peer=dst, offsets=[row, 0]
                        )
                        pl.tile.write(route_tile, [0, 0], pl.cast(route, pl.INT32))
                        pld.tile.remote_store(
                            route_tile, target=recv_route, peer=dst, offsets=[row, 0]
                        )
    ...
    with pl.spmd(
        ...
        # This expert block consumed every source lane for the epoch. N_LOCAL
        # increments per source form the reuse fence expected by its next push.
        for src in pl.range(N_RANKS):
            if src != my_rank:
                pld.system.notify(
                    target=data_arrived,
...
def combine(
    ...
    # routed_y_buf is route-keyed and reused every epoch. Wait until peers have
    # consumed our previous scatter before overwriting those rows.
    with pl.at(
        level=pl.Level.CORE_GROUP,
        name_hint="combine_reuse_wait",
        allow_early_resolve=True,
    ) as _combine_reuse_tid:
        # Anchor the wait to this layer's expert output so future layers cannot
        # occupy AIV cores while the ACK producers are still pending.
    ...
    # Acknowledge consumption only after every route row has been reduced, then
    # wait for the reciprocal ACK before allowing the output chain to retire.
    with pl.at(
        level=pl.Level.CORE_GROUP,
