...
def _prefill_cp_token_allgather_step(
    ...
    # Each peer block publishes its payload before its first-phase arrival.
    ...
    with pl.spmd(
        ...
        if tile_row + READBACK_ROW_TILE <= group_rows:
            window_tile = pl.load(gather_window, [tile_row, 0], [READBACK_ROW_TILE, D])
            ...
            for tail_offset in pl.range(READBACK_ROW_TILE):
                ...
                if tail_row < group_rows:
                    window_row = pl.load(gather_window, [tail_row, 0], [1, D])
